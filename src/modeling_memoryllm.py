...
class LlamaModel(LlamaPreTrainedModel, GenerationMixin):
    ...
    def _update_causal_mask(
        ...
        # ---- BEGIN PATCH: keep cache_position length == sequence_length ----
        sequence_length = input_tensor.shape[1]
        if cache_position.ndim != 1:
            cache_position = cache_position.view(-1)
        if cache_position.numel() != sequence_length:
            # keep the first T positions; aligns shapes for (T, S) * (T, 1)
        ...
        # print(f"sequence_length: {sequence_length}, target_length: {target_length}, past_seen_tokens: {past_seen_tokens}, attention_mask shape: {attention_mask.shape}")
...
class MemoryLLM(LlamaForCausalLM, GenerationMixin):
    def __init__(self, config):
        ...
        if config.lora_config is not None:
            ...
            # Minimal fix: PEFT expects `prepare_inputs_for_generation` on the base model it wraps.
            # We are passing the inner decoder (`self.model`, a LlamaModel) which doesn't define it.
            # Expose our LM version to the inner model before wrapping so PEFT can capture it.
            if not hasattr(self.model, "prepare_inputs_for_generation"):
                ...
                def _prepare_inputs_for_generation(
                    ...
                    **kwargs,
                    ...
                    return outer_self.prepare_inputs_for_generation(
                        input_ids=input_ids,
                        past_key_values=past_key_values,
                        attention_mask=attention_mask,
                        inputs_embeds=inputs_embeds,
                ...
                # Attach as a plain callable on the inner model instance
                self.model.prepare_inputs_for_generation = _prepare_inputs_for_generation
